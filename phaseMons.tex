...
TM01 is referred to as the monopole mode and TE11 as the dipole mode. The induced field distribution resulting from a bunch entering the cavity for both modes is shown in Figure~\ref{f:transModes}. The precise expressions for each field distribution can be found in [REF].

cylindrical cavity

notch filters create resonant volume and reject 12 GHz noise (e.g. wakefields) from outside that volume

beam pickup: rectangular slot then waveguide then feedthrough then 50 ohm coaxial to extract beam signal.

23mm internal diameter to limit number of transverse modes to two : TM01 (monopole) and TE11 (dipole) mode (cut off frequency below 12 GHz). 

Cut off frequency:
TEnm = c/2pi * pPrimenm / a (a is radium, pPrimenm is nth zero of derivative of bessel function)
TMmn = c/2pi * pnm / a (pnm is nth zero of bessel function)
pPrime11 = 1.84 (cutoff for TE11 7.6 GHz)
p01 = 2.4 (cutoff for TM01 9.96 GHz)
...
dipole mode can be removed by adding output of all 4 feedthroughs (why do we only add 2?), leaving only monopole. However, small misalignments in feedthroughs, cavity design etc. mean can't be removed completely.
...
\begin{figure}
  ...
  \caption{Picture of the two upstream phase monitors installed in the machine.}
  \label{f:phMonCTPic}
\end{figure}
\end{landscape}
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{Figures/phaseMons/phaseMonDiagram}
  \caption{Simplified schematic of the phase monitor electronics setup.}
...
latency monitor -> hybrid -> electronics -> out = 50 ns
...
The overall targeted bandwidth for the prototype PFF correction is 30~MHz, which means each individual hardware component must have a bandwidth in excess of 30~MHz. [TODO: What was the bandwidth target for the phase monitors? Some mentions of 100~MHz when talking about some reflection parameter in design paper. Maybe electronics is the main limitation?]
...
Figure~\ref{f:bandwidthPlot} shows the response of Mon~3 mixer to a beam pulse with a phase step setup in this way, where the first 140~ns of the pulse corresponds to the beam bypassing the delay loop and the second half of the pulse corresponding to the beam entering the delay loop.
...
\end{equation}
Where \(\mathrm{BW}\) is the bandwidth in MHz and \(t\) is the rise time of the signal in ns.
