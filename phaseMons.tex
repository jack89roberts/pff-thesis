...
\subsection{Single Sample Calibration Results}
\label{ss:calSingSamp}

Figure~\ref{f:allMixersSamp605} shows the result of fitting the mixer output versus the phase shifter setting at sample 605 along the pulse. The fitted values of the mixer amplitude, \(A\), and offset, \(d\), are found in Table~\ref{t:calFitConsts}. These values are used to calculate the beam phase as per Equation~\ref{e:phaseRecUsed}.

\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{Figures/phaseMons/allMixersSamp605}
  \caption{Fits to the mixer output vs. LO phase shifter setting at sample 605.}
  \label{f:allMixersSamp605}
\end{figure}
...
\begin{table}
  \begin{center}
    ...
    \end{tabular}
    \caption{Fit parameters from the calibration for each monitor.}
  	\label{t:calFitConsts}
  \end{center}
\end{table}
...
\subsection{Stability with Time}
\label{ss:calStability}
...
\newsection{resVsShifter}{Dependence of Resolution on LO Phase}
