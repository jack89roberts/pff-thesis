...
For zero length kickers, no quadrupole -- R22 = \(\pm1\), R12=0 works (R11 and R21 not needed as x=0 for zero length kicker). For non-zero length kicker with quadrupole around things more complicated -> MADX.

exit of first kicker to entrance of 2nd kicker:

\(x_{K2} = R_{11}x_{K1} + R_{12}x_{K1}'\)

\(x_{K2}' = R_{21}x_{K1} + R_{22}x_{K1}'\)

position end 1st kicker can be related to kick

\(x_{K1} = \alpha x_{K1}'\)

then equations above become

\(x_{K2} = \left(R_{11} + \frac{R_{12}}{\alpha}\right)x_{K1}\)

\(x_{K2}' = (\alpha R_{21} + R_{22})x_{K1}'\)

want \(x_{K2} = \mp x_{K1}\) and \(x_{K2}' = \pm x_{K1}'\), so:

\(R_{11} + \frac{R_{12}}{\alpha} = \mp 1\)

\(\alpha R_{21} + R_{22} = \pm 1\)

now many possible solutions. This doesn't consider quadrupole around 2nd kicker which is more difficult to model (split in to parts) and adds a perturbation to requirements for x, x' to be same at entrance to 2nd kicker as entrance to 1st kicker.
