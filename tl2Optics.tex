...
Each element of a beam line can be expressed as a transfer matrix \(\mathbf{R}\) (sometimes also called a response matrix) that defines how it transforms the initial coordinates of a particle in the beam [REF]:
\begin{equation}
...
Another way in which dipoles produce focusing effects is via their fringe fields. The fringe fields describe the increase in magnetic field from zero to the peak bending field as the beam approaches the entrance to the dipole, and from the peak field back to zero as the beam leaves the dipole [REF]. The extent and rate of increase of the fringe fields depends on the design of the dipole, in particular the pole separation [REF]. In MADX the nature of the fringe fields are defined by the variables HGAP, the pole separation, and FINT, which describes the shape of the increase up to the peak field (e.g. linear) [REF].

RBEND - distance in fringe field depends on position offset. Defocusing in bend plane. Focusing other plane.

Finally - add a pure quadrupole component.
